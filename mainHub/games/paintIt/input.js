...
function insideRect(mouseX, mouseY, rectPos) {
  if (
    mouseX >= innerWidth - rectPos &&
    mouseY >= rectPos / 2 &&
    mouseX <= innerWidth - rectPos + rectPos / 2 &&
    menuButton.menuHeight &&
    mouseY <= rectPos / 2 + menuButton.menuHeight
  ) {
    return true;
  } else {
    return false;
...
canvas.addEventListener("mouseup", (evt) => {
  if (editMode) {
    ...
    if (
      map[y][x] == "X" &&
      insideRect(mouse.x, mouse.y, menuButton.menuPos) == false
    ) {
      map[y][x] = ".";
    } else if (
      map[y][x] == "." &&
